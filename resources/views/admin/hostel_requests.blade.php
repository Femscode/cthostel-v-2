    ...
    <div class="g-5 gx-xxl-8">
        <div class="card">
            <div class="card-header border-0 pt-5">
                <h4>Requests for Hostel: {{ $hostel->name }} ({{ count($students) }})</h4>
                <div class="card-toolbar">
                    <a href="/hostels" class="btn btn-sm btn-light">Back to Hostels</a>
                </div>
            </div>
                ...
                <table id='datatable' class="table table-custom align-middle gs-0 gy-4">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$user->phone}}</td>
                            <td>
                                <span class="d-block fw-bold text-dark">{{ $user->school->name ?? "N/A" }}</span>
                            </td>
                            <td><a href='https://wa.me/234{{ substr($user->agent->phone ?? "000000", 1) }}' class="text-dark text-hover-primary">{{ $user->agent->name ?? "not-provided" }}</a></td>
